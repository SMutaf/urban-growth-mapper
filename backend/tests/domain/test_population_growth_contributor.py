...
def test_growing_district_contributes_positively():
    ...
    assert contributor.contribute(region, context) > 0
...
def test_missing_growth_rate_yields_zero():
    contributor = PopulationGrowthContributor()
    region = Region(id=1, name="r", city="sakarya", center_lat=40.0, center_lon=30.0)

    assert contributor.contribute(region, ScoringContext(region_growth_rates={})) == 0.0
