...
industrial zone (OSB), and port data from OpenStreetMap, replacing the
placeholder highway/railway/industrial-zone/port entries seeded earlier.
...
NOTE: bus stops are deliberately NOT handled here anymore - they're sourced
from Sakarya's own transit API instead (see scripts/ingest_sakarya_bus_stops.py),
which has far better coverage than OSM's crowdsourced stops. Re-running this
script no longer touches the bus_stop category, so it won't clobber that data.
...
from app.infrastructure.persistence.models import ProjectModel  # noqa: E402
...
def clear_superseded_placeholder_data(session) -> None:
    ...
    session.query(ProjectModel).filter(
        ...
        ProjectModel.project_type.in_(["highway", "railway", "industrial_zone", "port"]),
    ).delete(synchronize_session=False)
    session.commit()


def ingest() -> None:
    print("Querying Overpass for Sakarya transport + industrial zone + port data...")
